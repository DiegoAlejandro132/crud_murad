  ...
  <div class="list row">
    ...
    <div class="col-md-6">
      <div v-if="currentEndereco">
        <h4>Endereco</h4>
        <div>
          <label><strong>Nome:</strong></label> {{ currentEndereco.nomeEndereco }}
        </div>
        <div>
          <label><strong>CEP:</strong></label> {{ currentEndereco.cep }}
        </div>
        <div>
          <label><strong>Logradouro:</strong></label> {{ currentEndereco.logradouro }}
        </div>
        <div>
...
export default {
  ...
  methods: {
    ...
    searchTitle() {
      EnderecoDataService.findByTitle(this.nomeEndereco)
        .then(response => {
          this.enderecos = response.data;
          console.log(response.data);
        })
